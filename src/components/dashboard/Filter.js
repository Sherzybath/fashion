...
const Filter = () => {
    ...
    return (
                ...
                {['MEN', 'WOMEN', 'UNISEX'].map((gender) => (
                    <label key={gender}>
                        <input
                            ...
                            checked={selectedOption === gender.toLowerCase()}
                            onChange={() => handleOptionChange('gender', gender.toLowerCase())}
                        />
      ...
      <div ref={sectionsRefs.availability} className="dropdown-content" style={{ maxHeight: 0, overflow: 'hidden' }}>
          <label>
              <input
                  ...
                  onChange={() => handleOptionChange('availability', 'in-stock')}
              />
          ...
          <label>
              <input
                  ...
                  onChange={() => handleOptionChange('availability', 'out-of-stock')}
              />
          ...
          {['SMALL', 'MEDIUM', 'LARGE', 'EXTRA LARGE'].map((size) => (
              <label key={size}>
                  <input
                      ...
                      checked={selectedOption === size.toLowerCase()}
                      onChange={() => handleOptionChange('size', size.toLowerCase())}
                  />
      ...
      <div ref={sectionsRefs.collections} className="dropdown-content" style={{ maxHeight: 0, overflow: 'hidden' }}>
          {['SPRING', 'SUMMER', 'FALL', 'WINTER'].map((collection) => (
              <label key={collection}>
                  <input
                      type="radio"
                      name="collections"
                      value={collection.toLowerCase()}
                      checked={selectedOption === collection.toLowerCase()}
                      onChange={() => handleOptionChange('collections', collection.toLowerCase())}
                  />
      ...
      <div ref={sectionsRefs.clothing} className="dropdown-content" style={{ maxHeight: 0, overflow: 'hidden' }}>
          {['SEE ALL', 'DRESSES', 'HOODIES', 'JACKETS', 'KIMONO', 'PANTS', 'SWEATSHIRTS', 'T-SHIRTS'].map((item) => (
              <label key={item}>
                  <input
                      type="radio"
                      name="clothing"
                      ...
                      onChange={() => handleOptionChange('clothing', item.toLowerCase())}
                  />
      ...
      <div className="dropdown-header" onClick={() => toggleSection('pricing')}>
                ...
                {['UNDER $50', '$50 - $100', '$100 - $200', 'OVER $200'].map((priceRange) => (
                    <label key={priceRange}>
                        <input
                            ...
                            name="pricing"
                            value={priceRange}
                            checked={selectedOption === priceRange}
                            onChange={() => handleOptionChange('pricing', priceRange)}
                        />
            ...
            <div ref={sectionsRefs.dayDelivery} className="dropdown-content" style={{ maxHeight: 0, overflow: 'hidden' }}>
                {['SAME DAY', 'NEXT DAY', '2-3 DAYS', 'STANDARD (5-7 DAYS)'].map((delivery) => (
                    <label key={delivery}>
                        <input
                            type="radio"
                            name="dayDelivery"
                            ...
                            onChange={() => handleOptionChange('dayDelivery', delivery)}
                        />
